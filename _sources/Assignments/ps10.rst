...
* **Before class Monday 11/14:**
  ...
  * We will not go over the Twitter API in this class, *but*, we have provided some sample code that accesses it in case you are interested. The Twitter API requires ``requests_oauthlib``, and you need to do some setup in order to make an easy request to the API. See ``sample_twitter.py`` on Canvas > Code Samples if you are interested in investigating that yourself.
...
.. note::
    ...
    For some of the problems on this Problem Set, you'll have to write your own tests, which should check whether the problems follow the instructions! We suggest reading through the whole PSet first, and writing Unit Tests for each problem for which you need to do so, and *then* solving the problems themselves (test-driven development!).
...
.. external:: ps_10_03_test
    ...
    Write at least one unit test for Problem 3 in your file. 

    You can write it just beneath the space for your code, or you can add your new tests to the other ``unittest.TestCase`` subclasses near the bottom of the problem set that we've already provided for Problems 1 and 6. We recommend doing the latter -- model your tests off the tests you already see there and the ones you see in previous problem sets!
...
.. external:: ps_10_04_test
    ...
    Write at least 2 tests for Problem 4. (**Hint:** One of those tests should be checking to make sure that some of the elements are instances of class ``Post``!)
...
.. external:: ps_10_05_test
  ...
  Write at least 2 unit tests for Problem 5. 

  Note that you may not be able to test *whose* names should be in the list. Think: what DO you know about the values ``top_commenters`` and ``top_likers`` ought to end up with that you could check in a test?
...
.. external:: ps_10_06_test
    ...
    Add another test method to the ``Problem6`` class to test something else about the ``unique_facebookers`` function. 
