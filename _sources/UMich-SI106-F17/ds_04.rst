...
.. activecode:: ds_04_01
    ...
    **1.** Write code that counts the number of a's in ``word`` and store the result in the variable ``a_count``.
...
.. activecode:: ds_04_02
    ...
    **2.** The variable ``sentence`` stores a string.Write code to determine the number of words that contain the letter "c". Store the result in ``num_words_with_c``.
    ~~~~
...
.. activecode:: ds_04_03
    ...
    **3.** The variable ``sentence`` stores a string. Write code to determine the number of words that contain the letter "c" **or** are more than 5 characters long. Store the result in ``c_or_long``.
    ~~~~
    sentence = "students flock to the arb for a variety of outdoor activities such as jogging and picnicking"
...
.. activecode:: ds_04_04
    ...
    **4.** Below is a dictionary ``diction`` with two key-value pairs inside it. The string ``"python"`` is one of its keys. Using dictionary mechanics, print out the value of the key ``"python"``.
...
.. activecode:: ds_04_06
    ...
    **6.** Below is an empty dictionary saved in the variable ``nums``, and a list saved in the variable ``num_words``. Use iteration and dictionary mechanics to add each element of ``num_words`` as a key in the dictionary ``nums``. Each key should have the value ``0``. The dictionary should end up looking something like this when you print it out (remember, you can't be sure of the order): ``{"two":0,"three":0,"four":0,"eight":0,"seventeen":0,"not_a_number":0}``
    ~~~~
    nums = {}
...
.. activecode:: ds_04_07
    ...
    **7.** Suppose the dictionary ``golds`` maps countries to the number of gold medals they won in the 2016 olympics. Write code to store the names of countries that won more than 20 gold medals into the variable ``gold20``.
    ~~~~
    ...
    class myTests(TestCaseGui):
        def testOne(self):
            self.assertEqual(sorted(golds20),sorted(['USA', 'China', 'Canada', 'England']) , "Testing that golds20 has been created correctly.")
...
.. activecode:: ds_04_08
    ...
    **8.** Suppose the dictionary ``golds`` maps countries to the number of gold medals they won in the 2016 olympics. Write code to compute the name of the country that won the most gold medals. Store the result in the variable ``most_golds``
    ~~~~
    golds = {"Italy": 12, "USA": 33, "Brazil": 15, "China": 27, "Spain": 19, "Canada": 22, "Argentina": 8, "England": 29}
