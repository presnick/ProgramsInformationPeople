...
.. activecode:: ds_03_000
    :language: python
    :autograde: unittest
    :hidecode:

    Write code that assigns ``last_elem`` to the **last** element of the sequence ``S``. Don't hard-code your answer (in other words, if we changed ``S``, your answer should still be valid)
    ~~~~
    S = ["ab", "cd", "ef", "gh"]
...
.. activecode:: ds_03_05
    ...
    **5.** ``nums`` is a list of numbers. Write code that creates a new list, ``nums_squared``, that contains the square of every item in ``nums``. (Note: You can use either ``n*n`` or ``n**2`` to take the square of ``n``)
    ~~~~
    nums = [0, 3, 2, 5]
