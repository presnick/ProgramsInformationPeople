...
.. activecode:: ds_03_04
    ...
    **4.** ``nums`` is a list of numbers. Write code that creates a new list, ``nums_squared``, that contains the square of every item in ``nums``. (Note: You can use either ``n*n`` or ``n**2`` to take the square of ``n``)
    ~~~~
    nums = [0, 3, 2, 5]
...
.. activecode:: ds_03_05
    ...
    **5.** ``nums`` is a list of numbers. Write code to compute the average of every number in ``nums`` (the sum of every number divided by the number of items). Store your answer in ``avg_nums`` Your result should be a float. Do not hard code your answer (and do not hard code the number of items in ``nums``)
    ~~~~
    nums = [10, 20, 30, 50]
...
.. activecode:: ds_03_06
    ...
    **6.** ``rainfall_mi`` is a string that contains the average number of inches of rainfall in Michigan for every month (in inches) with every month separated by a comma. Write code to compute the total rainfall (by adding the value for every month) by taking the sum of every number of ``rainfall_mi``. Store the total rainfall in the variable ``yearly_rainfall_mi``.
    The, write code to compute the monthly rainfall by computing the average of every number of ``rainfall_mi``. Store that average in the variable ``monthly_rainfall_mi``. Do not hard-code your answer.
    ~~~~
...
.. activecode:: ds_03_07
    ...
    **7.** Write code that uses the turtle framework to draw the following, using a for loop:
