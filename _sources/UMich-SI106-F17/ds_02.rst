...
.. activecode:: ds_02_01
    ...
    **1.** Suppose the ``sqrt()`` function returns the square root of its arguments. Write a statement that will assign a variable ``c`` to the square root of ``a`` squared plus the value of ``b`` squared (Pythagorean theorem).
    ...
    class myTests(TestCaseGui):
        def testOne(self):
            self.assertAlmostEqual(c, 5.0, "Testing that c has the correct value")
...
.. activecode:: ds_02_03
    ...
    **3.** Write code that asks the user to enter a temperature in Fahrenheit and prints that temperature in Celsius. The formula to convert Fahrenheit to Celsius is: C = (F-32)*5/9
    ~~~~
