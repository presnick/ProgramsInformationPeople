...
.. mchoice:: Saving_json_in_text_file
   :practice: T
...
.. mchoice:: Obtaining_Saving_Credentials_First_Time
   ...
   What happens if we run the following code for the first time?
   ...
   .. code-block:: python
        ...
        try:
            ...
            f.close()
            print("Read the credentials from the file.")
        except:
            # If not, you'll have to get them
            # and then save them in creds.txt
            tokens = get_tokens()
            ...
            f.close()
            (client_key, client_secret, resource_owner_key, resource_owner_secret, verifier) = tokens
            print("Got the credentials from get_tokens() and saved them in the file.")
