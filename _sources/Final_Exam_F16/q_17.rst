   ...
   **tweet_data = json.loads(s)**. We have provided you with the value that **json.loads(s)** has returned.
   ...
   class myTests(TestCaseGui):
     ...
     def testOne(self):
         ...
         self.assertEqual(str(popular_tweeters(tweet_instances)), "[<Tweet instance for jackiec>, <Tweet instance for tsuyoshi_13>, <Tweet instance for samc_99>]", "Testing whether `str(popular_tweeters(tweet_instances))` returns the right value.")
